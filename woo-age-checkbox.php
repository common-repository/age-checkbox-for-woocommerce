...
function bcWOAG_age_policy_form() {
    if(get_option( 'woocommerce_betawooage_checkbox' )=='yes'){
      ...
      if(get_option( 'woocommerce_betawooage_multilanguage' )=='yes'){
        $checkText = __( "You have to be 18 years or older to order products at this webshop. By checking this box you confirm that you are 18 years or older.", 'betawooage' );
      }else{
        $checkText = get_option( 'woocommerce_betawooage_checkboxemessage' );
      }
...
function bcWOAG_not_approved_age() {
    if(get_option( 'woocommerce_betawooage_checkbox' )=='yes'){
        if ( ! (int) isset( $_POST['age_policy'] ) ) { 
          ...
          if(get_option( 'woocommerce_betawooage_multilanguage' )=='yes'){
            $failText = __( "<strong>Please confirm that you are 18 years or older</strong> by ticking the box!", 'betawooage' );
          }else{
            $failText = get_option( 'woocommerce_betawooage_failuremessage' );
          }

          wc_add_notice( $failText, 'error' );
...
function bcWOAG_form_wpadmin( $settings ) {
  ...
  foreach ( $settings as $section ) {
    ...
    if ( isset( $section['id'] ) && 'general_options' == $section['id'] &&
      ...
      $newsettings[] = array(
        'name'     => __( 'Multilanguage age checkbox', 'betawooage' ),
        'id'       => 'woocommerce_betawooage_multilanguage',
        'type'     => 'checkbox',
        'css'      => '',
        'std'      => 1,  // WC < 2.0
        'default'  => 1,  // WC >= 2.0
        'desc'     => __( "Refer to the known translations of this plugin or create your own by using another plugin.", 'betawooage' ),
      );
      if(get_option( 'woocommerce_betawooage_multilanguage' )=='no' OR get_option( 'woocommerce_betawooage_multilanguage' )==''){
         $newsettings[] = array(
            'name'     => __( 'Checkbox message', 'betawooage' ),
            'id'       => 'woocommerce_betawooage_checkboxemessage',
            'type'     => 'textarea',
            'css'      => 'min-width:300px; min-height:100px;',
            'std'      => "You have to be 18 years or older to order products at this webshop. By checking this box you confirm that you are 18 years or older.",  // WC < 2.0
            'default'  => "You have to be 18 years or older to order products at this webshop. By checking this box you confirm that you are 18 years or older.",  // WC >= 2.0
         ...
         $newsettings[] = array(
            ...
            'type'     => 'textarea',
            'css'      => 'min-width:300px; min-height:100px;',
            'std'      => "<strong>Please confirm that you are 18 years or older</strong> by ticking the box.",  // WC < 2.0
            'default'  => "<strong>Please confirm that you are 18 years or older</strong> by ticking the box.",  // WC >= 2.0
